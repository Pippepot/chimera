...
from chimera.graph import Node, Print, BinaryOp, Index, Array, Var, Assign, Const, Loop, Function, Call, PatternMatcher, Pat
...
render_patterns = PatternMatcher([
  ...
  (Pat(Index, name='x'), lambda ctx, x: f"*({ctx[x.data]}+{ctx[x.indices[0]]})"), # do strides in future and loop over all indices
